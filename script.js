...
document.getElementById("message").addEventListener("input", function () {
  const rawMessage = this.value;
  console.log("[UI] Message input updated:", rawMessage);
  const formattedMessage = formatMessage(rawMessage);
  document.getElementById("messagePreview").innerHTML = formattedMessage;
});
...
document.getElementById("image").addEventListener("change", function () {
  ...
  if (file) {
    ...
    reader.onload = function (e) {
      ...
      console.log("[UI] Image preview updated");
    };
    reader.onerror = (e) => console.error("[UI] Image preview error:", e);
    reader.readAsDataURL(file);
  } else {
...
function listenToProgress() {
  if (eventSource) {
    console.log(
      "[SSE] EventSource already active. Reusing existing connection."
    );
    return;
  }
  ...
  eventSource.onmessage = (event) => {
    try {
      const data = JSON.parse(event.data);
      console.log("[SSE] Progress data received:", data);
      ...
      if (data.status === "qr") {
        ...
        statusDiv.appendChild(message);
        const messages = statusDiv.getElementsByTagName("p");
        if (messages.length > 10) statusDiv.removeChild(messages[0]);
      }

      if (
        data.status === "progress" &&
        data.message?.includes("Message sent")
      ) {
        const counterElement = document.getElementById("counter");
        const currentCount = parseInt(counterElement.innerText || "0", 10);
        counterElement.innerText = currentCount + 1;
        console.log("[UI] Message counter incremented:", currentCount + 1);
      }

      if (data.status === "success" || data.status === "error") {
        console.log("[SSE] Final status received:", data.status);
        const sendButton = document.getElementById("sendButton");
        sendButton.disabled = false;
        sendButton.innerText = "Send Messages";
        eventSource.close();
  ...
  eventSource.onerror = (error) => {
    console.error("[SSE] Error receiving progress updates:", error);
    const sendButton = document.getElementById("sendButton");
    sendButton.disabled = false;
    sendButton.innerText = "Send Messages";
    try {
...
document
  ...
  .addEventListener("submit", function (event) {
    ...
    console.log("[UI] Form submitted. Sending data to server...");
    listenToProgress();
    ...
    fetch("http://localhost:3000/send-messages", {
      ...
      .then((data) => {
        console.log("[HTTP] Response JSON:", data);
        if (data.status === "success") {
          console.log("[UI] Batch is being processed.");
        } else {
          sendButton.disabled = false;
          sendButton.innerText = "Send Messages";
        }
      ...
      .catch((error) => {
        console.error("[HTTP] Error submitting form:", error);
        document.getElementById("status").innerText = "An error occurred.";
        sendButton.disabled = false;
        sendButton.innerText = "Send Messages";
      });
